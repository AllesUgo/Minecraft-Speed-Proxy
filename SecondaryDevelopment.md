...
## 项目架构及注意事项（重点！！！）
- 代理框架的核心类是Proxy类及其依赖的其他文件和类，建议任何二次开发基于调用Proxy类的成员方法进行开发而非直接修改Proxy类的源码。
- Proxy类的其他成员方法应在服务启动后调用，服务启动前调用可能会导致卡死或未定义行为（其他异步执行器将在服务启动后开始工作）。
- Proxy类的属性、回调函数应在服务启动前设置，否则可能会导致未定义行为。
- 遇到本文档和代码不一致的情况，请以代码为准。文档更新可能延迟。
- 在开发前务必阅读proxy.h中Proxy类的声明及注释，遵循注释中的注意事项。
## 类和函数
### ::Proxy类
...
|int GetMaxPlayer(void)|获取最大在线用户数|
...
|auto GetDefaultProxy() -> std::pair<std::string, std::uint16_t>;|获取默认的代理，也就是配置文件中给出的代理|
